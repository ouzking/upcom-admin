...
/** Écran explicite plutôt qu'une application cassée si les variables Supabase manquent (local ou hébergeur). */
      ...
      <div className="max-w-lg rounded-2xl border border-line bg-paper p-8 shadow-card">
        ...
        {import.meta.env.PROD ? (
          <p className="mt-2 text-sm text-muted">
            Ce déploiement a été construit sans les variables <code className="rounded bg-mist px-1.5 py-0.5">VITE_SUPABASE_URL</code> et{" "}
            <code className="rounded bg-mist px-1.5 py-0.5">VITE_SUPABASE_PUBLISHABLE_KEY</code>. Ajoutez-les dans les variables d'environnement de
            l'hébergeur (Netlify : Site configuration → Environment variables), puis relancez un déploiement : elles sont intégrées au moment du build.
          </p>
        ) : (
          <p className="mt-2 text-sm text-muted">
